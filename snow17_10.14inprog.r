...
df <- df %>% mutate(date = mdy(date), tavg_c = ((tmax_c + tmin_c)/2), doy = yday(date)) %>% 
      filter(date > "1982-8-16", date < "1983-8-16")
...
snow17 <- function(par, prcp, tavg, elev, doy,
          ini.tstep.state =
          c(0,           0,              0,    0,                 0,            0))        {
          #we_solid[mm]  #we_liquid[mm]  #ati  #heatdeficit[mm]   #swe         #si_temp - temp 100% cover
...
#SBWS = Water equivalent above which 100 percent areal snow cover
#temporarily exists (units of MM)
#
...
# define water
if (swe >= hsnof * ts_p) {
  si_temporarilycovered  <- swe + 0.75 * we_newsnow_gadj } #[mm]
else {
  si_temporarilycovered  <- si_temporarilycovered + 0.75 * we_newsnow_gadj } #[mm]
 }
...
if (we_solid > daygm) { # if more swe than the daily ground melt rate assumed
  ...
  swe <- we_solid + we_liquid
    ...
    ini.tstep.state <- c(we_solid, we_liquid, ati, heatdef, swe = we_solid + we_liquid, si_temp)
