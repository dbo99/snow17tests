...
df <- df %>% mutate(tstep = mdy(date), tavg_c = ((tmax_c + tmin_c)/2), doy = yday(tstep)) %>% 
...
#tstep <- seq(from=as.POSIXct("1984-10-1 0:00", tz="UTC"), to=as.POSIXct("2010-10-1 00:00", tz="UTC"), by="6 hours")  
#csv_mat <- read_csv("sbrc1up_mat.csv") 
#mat6hr_degC <- c(na.omit(c(t(csv_mat))))[1:length(tstep)]
#csv_map <- read_csv("sbrc1up_map.csv")
#map6hr_mm <- c(na.omit(c(t(csv_map))))[1:length(tstep)] 
#
#df <- data.frame(tstep, mat6hr_degC, map6hr_mm) %>% mutate(doy = yday(tstep))
#
##input timestep interval in hours, current setup requires each to be the same
#ts_t <- 6  # temperature [C]
#ts_p <- 6  # precipitation [mm]
...
meanarealwe_to_ai_x <- c(0.0, 0.05, .09, .15, .23, .37, .56, .72, .86, .93, 1)
...
if (we_newsnow_gadj > 1.5 * ts_p) {
 ...
 tipm_ts_t <- 1 - ((1 - tipm)^(ts_t/6))  #formula based on 6 hr timestep, accommodates any factor of 6
...
ggplot(df, aes(tstep, arealextent)) + geom_line()
#ggplotly(p)
